...
- 新建项目可选择手动模式或全自动模式；全自动任务点击提交后先进入 SQLite 持久化 FIFO 队列并显示“上传任务素材”，素材齐全后才执行 AI 导演、音频、对齐和视频；AI 导演失败后每 3 秒持续重试。
...
根目录 `run.ps1` 默认同时启动后端和前端，前端地址为
<http://127.0.0.1:5173>。首次启动会自动在 `frontend/` 安装 npm 依赖，不再需要
HBuilderX。只启动后端可运行 `.\run.ps1 -BackendOnly`。
...
前端开发服务器监听 `0.0.0.0:5173`。同一局域网的其他设备可以打开：

```text
http://服务器局域网IP:5173
```

H5 默认请求同源 `/api`，Vite 会在服务器内部代理到
`http://127.0.0.1:8000`。远程浏览器不需要直接访问后端端口，也不会把
`127.0.0.1` 误认为服务器。

如果 Windows 防火墙尚未放行，需要使用管理员 PowerShell根据实际网络安全策略开放
TCP `5173`。本项目不会自动修改系统防火墙。

公网部署建议使用 HTTPS、Nginx 或 VPN，并增加身份认证。当前 API 具备生成任务和
删除项目能力，不建议直接把开发服务器或后端端口裸露到公网。
...
创建任务时勾选“视频生成后添加背景音乐”，可手动上传指定音乐，也可从系统设置中
配置的服务器音乐文件夹随机选择。随机模式在任务创建时选定一首并复制进项目目录，
不会在排队或生成过程中更换。背景音乐支持 WAV、FLAC、MP3、M4A、M4S、MP4、
OGG 和 AAC；手动上传单文件上限 200MB。后端使用本机 `ffmpeg` 完成混音，不占用
ComfyUI GPU。输出文件包括：
...
InfiniteTalk。若混音失败，项目显示“配乐失败”，无配乐原视频仍可播放和下载。生成配乐版后，页面同时提供配乐成片和无配乐版下载；无配乐版优先保留已经添加的标题与字幕。
