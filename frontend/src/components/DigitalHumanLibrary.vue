      ...
      <view class="panel character-editor">
        <view class="character-editor-layout">
          ...
          <view class="character-editor-form">
            <text class="label">配置名称</text><input v-model="draft.name" class="field" maxlength="100" placeholder="例如：陈彬" />
            <text class="label">备注</text><textarea v-model="draft.note" class="textarea character-note" maxlength="500" placeholder="可填写人物、用途或声音特点"></textarea>
            <text class="label">默认配音方式</text>
            <view class="engine-options"><button class="engine-option" :class="{active:draft.default_tts_engine==='indextts2_legacy'}" @click="draft.default_tts_engine='indextts2_legacy'"><text>情绪参数配音</text></button><button class="engine-option" :class="{active:draft.default_tts_engine==='indextts2_voice_clone'}" @click="draft.default_tts_engine='indextts2_voice_clone'"><text>音色与情感参考</text></button></view>
          </view>
        </view>
        <view class="uploads character-editor-assets">
          ...
          <view class="upload-box" :class="{selected:Boolean(emotionVoicePath||editingProfile?.has_emotion_voice)}" @click="$emit('choose-asset','emotion_voice')"><text class="upload-icon">♫</text><text>情感参考音频</text><text class="upload-status">{{ emotionVoicePath ? '✓ 已选择新参考' : (editingProfile?.has_emotion_voice ? '✓ 已保存' : '可选') }}</text><text class="hint file-name">{{ emotionVoiceFileName || '音色与情感参考模式下必需' }}</text></view>
        </view>
...
export default {
  ...
  emits:['create','open','back','save','delete','choose-asset'],
