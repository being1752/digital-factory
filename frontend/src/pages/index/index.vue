...
export default {
  ...
  computed: {
    ...
    selectedCharacterProfile(){return this.characterProfiles.find(item=>item.id===this.selectedCharacterProfileId)||null},
    selectedCharacterProfileImage(){return this.selectedCharacterProfile?.has_image?fileUrl(this.selectedCharacterProfile.image_url):''},
    ...
    createChecks(){const profile=this.selectedCharacterProfile,clone=this.form.tts_engine==='indextts2_voice_clone',hasImage=Boolean(this.imagePath||profile?.has_image),hasVoice=Boolean(this.voicePath||profile?.has_voice),hasEmotion=Boolean(this.emotionVoicePath||profile?.has_emotion_voice),checks=[{label:'视频上方标题',ok:!this.form.video_title_enabled||Boolean(String(this.form.video_title||'').trim()),required:Boolean(this.form.video_title_enabled),detail:this.form.video_title_enabled?(String(this.form.video_title||'').trim()?'已填写':'已开启标题，请填写内容'):'未开启'},{label:'口播文案',ok:Boolean(String(this.form.original_script||'').trim()),required:true,detail:String(this.form.original_script||'').trim()?'已填写':'必填'},{label:'数字人图片',ok:hasImage,required:false,detail:this.imagePath?(this.imageFileName||'已选择'):(profile?.has_image?'来自角色配置':'未选择时使用默认图片')},{label:'音色参考音频',ok:hasVoice,required:false,detail:this.voicePath?(this.voiceFileName||'已选择'):(profile?.has_voice?'来自角色配置':'未选择时使用默认音色')}];if(clone)checks.push({label:'情感参考音频',ok:hasEmotion,required:true,detail:this.emotionVoicePath?(this.emotionVoiceFileName||'已选择'):(profile?.has_emotion_voice?'来自角色配置':'当前配音方式需要此文件')});if(this.form.bgm_enabled){const random=this.form.bgm_source==='library_random';checks.push({label:'背景音乐',ok:random||Boolean(this.bgmPath),required:true,detail:random?(this.bgmRandomName||'提交时自动选择，可先试听'):(this.bgmPath?(this.bgmFileName||'已选择'):'已开启配乐，请选择文件')})};return checks},
  ...
  methods: {
    ...
    async connectBackend(){let ready=false;try{this.health=await request('/api/health');await Promise.all([this.loadAppSettings(),this.loadFonts(),this.loadProjects(),this.loadTasks(),this.loadCharacterProfiles()]);ready=true}catch(e){this.toast(e.message)}finally{this._needsRealtimeResync=!ready;this.startRealtimeUpdates()}},
