  ...
  <view class="page">
    ...
    <view v-if="viewMode==='create'||viewMode==='projects'" class="shell" :class="{'create-shell':viewMode==='create'}">
      ...
      <view class="workspace">
        ...
        <CreateTask v-if="viewMode==='create'" :form="form" :create-panels="createPanels" :font-options="fontOptions" :subtitle-colors="subtitleColors" :subtitle-positions="subtitlePositions" :script-summary="scriptSummary" :create-checks="createChecks" :create-ready="createReady" :image-path="imagePath" :image-file-name="imageFileName" :voice-path="voicePath" :voice-file-name="voiceFileName" :emotion-voice-path="emotionVoicePath" :emotion-voice-file-name="emotionVoiceFileName" :bgm-path="bgmPath" :bgm-file-name="bgmFileName" :bgm-random-name="bgmRandomName" :bgm-preview-playing="bgmPreviewPlaying" :submitting="submitting" :upload-label="uploadLabel" :upload-progress="uploadProgress" @set-title="setProjectTitle" @choose-image="chooseImage" @choose-voice="chooseVoice" @choose-emotion-voice="chooseEmotionVoice" @choose-bgm="chooseBgm" @choose-random-bgm-preview="chooseRandomBgmPreview" @preview-bgm="toggleCreateBgmPreview" @stop-bgm-preview="resetBgmPreview" @update-bgm-preview-volume="updateBgmPreviewVolume" @toggle-panel="toggleCreatePanel" @set-auto-run="setAutoRun" @set-bgm-enabled="setBgmEnabled" @set-bgm-ducking="setBgmDucking" @set-title-enabled="setVideoTitleEnabled($event,form)" @set-subtitle-enabled="setSubtitleEnabled" @set-subtitle-bold="setSubtitleBold($event,form)" @set-subtitle-background="setSubtitleBackground" @apply-title-preset="applyChannelsTitlePreset(form)" @apply-subtitle-preset="applyChannelsSubtitlePreset(form)" @submit="createProject" />
        ...
        <template v-if="viewMode==='projects'&&current">
          ...
          <PostEditor v-if="current.has_video&&projectTab==='post'" :current="current" :video-source="videoSource" :is-busy="isBusy" :post-panels="postPanels" :font-options="fontOptions" :subtitle-colors="subtitleColors" :subtitle-positions="subtitlePositions" :image-url="assetUrl('image')" :bgm-preview-playing="bgmPreviewPlaying" @download="downloadVideo" @load-video="loadVideo" @video-error="videoLoadError" @toggle-panel="togglePostPanel" @apply-title-preset="applyChannelsTitlePreset(current)" @set-title-enabled="setCurrentVideoTitleEnabled" @apply-subtitle-preset="applyChannelsSubtitlePreset(current)" @set-subtitle-enabled="setCurrentSubtitleEnabled" @set-subtitle-bold="setSubtitleBold($event,current)" @set-subtitle-background="setCurrentSubtitleBackground" @save-subtitle="saveSubtitleAndRun" @choose-bgm="chooseProjectBgm" @choose-random-bgm="chooseRandomProjectBgm" @preview-bgm="toggleProjectBgmPreview" @update-bgm-preview-volume="updateBgmPreviewVolume" @set-bgm-ducking="setCurrentBgmDucking" @save-bgm="saveBgmAndRun" />
...
export default {
  ...
  data() { return {viewMode:'create',projectTab:'overview',pageVisible:true,realtimeConnected:false,uploadProgress:0,uploadLabel:'',health:{},appSettings:{comfy_url:'',music_library_path:'',music_library_count:0},settingsSaving:false,availableFonts:[],projects:[],tasks:[],current:null,poll:null,queuePoll:null,submitting:false,comfyChecking:false,comfyCheckResult:'',comfyCheckOk:false,imagePath:'',imageFileName:'',voicePath:'',voiceFileName:'',emotionVoicePath:'',emotionVoiceFileName:'',bgmPath:'',bgmFileName:'',bgmRandomName:'',bgmPreviewPlaying:false,bgmPreviewSource:'',audioPlaying:false,audioCurrent:0,audioTotal:0,audioSource:'',videoSource:'',createPanels:{title:false,subtitle:false,bgm:false},postPanels:{title:false,subtitle:false},emotionNames:['Happy','Angry','Sad','Fear','Hate','Low','Surprise','Neutral'],form:{title:'健康管理口播',tts_engine:'indextts2_legacy',original_script:'百万亿健康管理蓝海市场，机遇就在眼前。友福同享智能科技有限公司，专注一站式AI健康管理五年多。现面向全国招募社区健康服务中心项目合伙人。如果你对健康管理感兴趣，想低门槛撬动高价值、高利润项目，友福就是你的最佳选择。友福三大核心优势，帮合伙人轻松开拓市场。',auto_run:true,bgm_enabled:true,bgm_source:'library_random',bgm_library_name:null,bgm_volume:0.25,bgm_ducking:true,bgm_fade_in:1.5,bgm_fade_out:2}} },
  computed: {
    ...
    createChecks(){const clone=this.form.tts_engine==='indextts2_voice_clone',checks=[{label:'视频上方标题',ok:!this.form.video_title_enabled||Boolean(String(this.form.video_title||'').trim()),required:Boolean(this.form.video_title_enabled),detail:this.form.video_title_enabled?(String(this.form.video_title||'').trim()?'已填写':'已开启标题，请填写内容'):'未开启'},{label:'口播文案',ok:Boolean(String(this.form.original_script||'').trim()),required:true,detail:String(this.form.original_script||'').trim()?'已填写':'必填'},{label:'数字人图片',ok:Boolean(this.imagePath),required:false,detail:this.imagePath?(this.imageFileName||'已选择'):'未选择时使用默认图片'},{label:'音色参考音频',ok:Boolean(this.voicePath),required:false,detail:this.voicePath?(this.voiceFileName||'已选择'):'未选择时使用默认音色'}];if(clone)checks.push({label:'情感参考音频',ok:Boolean(this.emotionVoicePath),required:true,detail:this.emotionVoicePath?(this.emotionVoiceFileName||'已选择'):'当前配音方式需要此文件'});if(this.form.bgm_enabled){const random=this.form.bgm_source==='library_random';checks.push({label:'背景音乐',ok:random||Boolean(this.bgmPath),required:true,detail:random?(this.bgmRandomName||'提交时自动选择，可先试听'):(this.bgmPath?(this.bgmFileName||'已选择'):'已开启配乐，请选择文件')})};return checks},
  ...
  methods: {
    ...
    async openProject(id){clearInterval(this.poll);this.resetAudio();this.resetBgmPreview();this.resetVideo();this.postPanels={title:false,subtitle:false};this.viewMode='projects';this.projectTab='overview';this.current=await request(`/api/projects/${id}`);this.startPolling()},
    ...
    chooseBgm(){uni.chooseFile({count:1,extension:['wav','flac','mp3','m4a','m4s','mp4','ogg','aac'],success:r=>{this.resetBgmPreview();this.bgmPath=r.tempFilePaths[0];this.bgmFileName=r.tempFiles?.[0]?.name||'已选择背景音乐'}})},
    async chooseRandomBgmPreview(){try{const item=await request('/api/music/random',{method:'POST'});this.bgmRandomName=item.name;this.form.bgm_library_name=item.name;this.playBgmPreview(fileUrl(`/api/music/preview?name=${encodeURIComponent(item.name)}`),this.form.bgm_volume)}catch(error){this.toast(error.message)}},
    toggleCreateBgmPreview(){const source=this.form.bgm_source==='library_random'?(this.bgmRandomName?fileUrl(`/api/music/preview?name=${encodeURIComponent(this.bgmRandomName)}`):''):this.bgmPath;if(!source){this.toast('请先选择一首背景音乐');return}this.toggleBgmPreview(source,this.form.bgm_volume)},
    toggleProjectBgmPreview(){if(!this.current?.has_bgm){this.toast('请先选择一首背景音乐');return}this.toggleBgmPreview(this.assetUrl('bgm'),this.current.bgm_volume)},
    playBgmPreview(source,volume){this.resetBgmPreview();const context=uni.createInnerAudioContext();this._bgmPreviewContext=context;this.bgmPreviewSource=source;context.src=source;context.volume=Math.max(0,Math.min(1,Number(volume??0.25)));context.onPlay(()=>{this.bgmPreviewPlaying=true});context.onPause(()=>{this.bgmPreviewPlaying=false});context.onStop(()=>{this.bgmPreviewPlaying=false});context.onEnded(()=>{this.bgmPreviewPlaying=false});context.onError(error=>{this.bgmPreviewPlaying=false;this.toast(error.errMsg||'背景音乐试听失败')});context.play()},
    toggleBgmPreview(source,volume){if(this._bgmPreviewContext&&this.bgmPreviewSource===source){if(this.bgmPreviewPlaying)this._bgmPreviewContext.pause();else this._bgmPreviewContext.play();return}this.playBgmPreview(source,volume)},
    updateBgmPreviewVolume(value){if(this._bgmPreviewContext)this._bgmPreviewContext.volume=Math.max(0,Math.min(1,Number(value||0)))},
    resetBgmPreview(){if(this._bgmPreviewContext){this._bgmPreviewContext.stop();this._bgmPreviewContext.destroy();this._bgmPreviewContext=null}this.bgmPreviewPlaying=false;this.bgmPreviewSource=''},
    setAutoRun(event){this.form.auto_run=(event?.detail?.value||[]).includes('auto')},
    setBgmEnabled(event){this.form.bgm_enabled=(event?.detail?.value||[]).includes('bgm');if(!this.form.bgm_enabled)this.resetBgmPreview()},
    ...
    applyChannelsTitlePreset(target){Object.assign(target,{video_title_enabled:true,video_title_font_name:'Microsoft YaHei',video_title_font_size:88,video_title_primary_color:'#FFFFFF',video_title_secondary_color:'#FFD84D',video_title_tertiary_color:'#7DE3FF',video_title_position:10,video_title_stroke_color:'#000000',video_title_stroke_width:4});this.toast('已应用视频号顶部标题预设','success')},
    ...
    async createProject(){this.uploadProgress=0;this.uploadLabel='';if(this.form.video_title_enabled&&!String(this.form.video_title||'').trim()){this.toast('请填写视频上方标题');return}if(!this.form.original_script.trim()){this.toast('请填写口播文案');return}const needsEmotion=this.form.tts_engine==='indextts2_voice_clone',manualBgm=this.form.bgm_enabled&&this.form.bgm_source!=='library_random';if(needsEmotion&&!this.emotionVoicePath){this.toast('请选择情感参考音频');return}if(manualBgm&&!this.bgmPath){this.toast('手动配乐模式下请选择背景音乐');return}const autoRun=Boolean(this.form.auto_run);let task=null;this.submitting=true;try{const createPayload={...this.form,expect_image_upload:Boolean(this.imagePath),expect_voice_upload:Boolean(this.voicePath),expect_emotion_voice_upload:Boolean(this.emotionVoicePath),expect_bgm_upload:Boolean(manualBgm&&this.bgmPath)};let p=await request('/api/projects/default',{method:'POST',data:createPayload});if(autoRun){task=await request(`/api/projects/${p.id}/enqueue`,{method:'POST'});await this.loadTasks()}try{if(this.imagePath){this.uploadLabel='正在上传数字人图片';p=await upload(`/api/projects/${p.id}/assets/image`,this.imagePath,'file',value=>this.uploadProgress=value)}if(this.voicePath){this.uploadLabel='正在上传参考声音';p=await upload(`/api/projects/${p.id}/assets/voice`,this.voicePath,'file',value=>this.uploadProgress=value)}if(this.emotionVoicePath){this.uploadLabel='正在上传情感音频';p=await upload(`/api/projects/${p.id}/assets/emotion_voice`,this.emotionVoicePath,'file',value=>this.uploadProgress=value)}if(manualBgm&&this.bgmPath){this.uploadLabel='正在上传背景音乐';p=await upload(`/api/projects/${p.id}/assets/bgm`,this.bgmPath,'file',value=>this.uploadProgress=value)}if(!p.has_image||!p.has_voice||(needsEmotion&&!p.has_emotion_voice)||(this.form.bgm_enabled&&!p.has_bgm))throw new Error('图片、音频或背景音乐素材未上传成功')}catch(uploadError){if(task)try{await request(`/api/tasks/${task.id}/cancel`,{method:'POST'})}catch(_){}throw uploadError}if(autoRun)this.current=null;else this.current=p;this.imagePath='';this.imageFileName='';this.voicePath='';this.voiceFileName='';this.emotionVoicePath='';this.emotionVoiceFileName='';this.bgmPath='';this.bgmFileName='';this.bgmRandomName='';this.form.bgm_library_name=null;this.resetBgmPreview();await Promise.all([this.loadProjects(),this.loadTasks()]);if(autoRun)this.toast('任务已入队，素材上传完成','success');else this.toast('项目已创建，请按步骤手动操作','success')}catch(e){this.toast(e.message)}finally{this.submitting=false;this.uploadProgress=0;this.uploadLabel=''}},
    ...
    chooseProjectBgm(){uni.chooseFile({count:1,extension:['wav','flac','mp3','m4a','m4s','mp4','ogg','aac'],success:async r=>{try{this.resetBgmPreview();this.current=await upload(`/api/projects/${this.current.id}/assets/bgm`,r.tempFilePaths[0]);this.resetVideo();this.toast('背景音乐已上传，可以先试听再生成配乐版','success')}catch(error){this.toast(error.message)}}})},
    async chooseRandomProjectBgm(){try{this.resetBgmPreview();this.current=await request(`/api/projects/${this.current.id}/bgm/random`,{method:'POST'});this.resetVideo();this.toast(`已自动选择：${this.current.bgm_name||'背景音乐'}，可以先试听`,'success')}catch(error){this.toast(error.message)}},
    ...
    async saveSubtitleAndRun(){try{
      const fields=['subtitle_enabled','subtitle_font_name','subtitle_font_size','subtitle_font_bold','subtitle_font_color','subtitle_position','subtitle_custom_position','subtitle_stroke_color','subtitle_stroke_width','subtitle_background_enabled','subtitle_background_color','subtitle_background_opacity','subtitle_max_chars','video_title_enabled','video_title','video_title_font_name','video_title_font_size','video_title_primary_color','video_title_secondary_color','video_title_tertiary_color','video_title_position','video_title_stroke_color','video_title_stroke_width'],numeric=new Set(['subtitle_font_size','subtitle_custom_position','subtitle_stroke_width','subtitle_background_opacity','subtitle_max_chars','video_title_font_size','video_title_position','video_title_stroke_width'])
