...
export default {
  ...
  data() { return {viewMode:'create',projectTab:'overview',pageVisible:true,realtimeConnected:false,uploadProgress:0,uploadLabel:'',health:{},appSettings:{comfy_url:'',music_library_path:'',music_library_count:0},settingsSaving:false,availableFonts:[],projects:[],tasks:[],current:null,poll:null,queuePoll:null,submitting:false,comfyChecking:false,comfyCheckResult:'',comfyCheckOk:false,imagePath:'',imageFileName:'',voicePath:'',voiceFileName:'',emotionVoicePath:'',emotionVoiceFileName:'',bgmPath:'',bgmFileName:'',audioPlaying:false,audioCurrent:0,audioTotal:0,audioSource:'',videoSource:'',lastSuggestedVideoTitle:'',createPanels:{title:false,subtitle:false,bgm:false},postPanels:{title:false,subtitle:false},emotionNames:['Happy','Angry','Sad','Fear','Hate','Low','Surprise','Neutral'],form:{title:'健康管理口播',tts_engine:'indextts2_legacy',original_script:'百万亿健康管理蓝海市场，机遇就在眼前。友福同享智能科技有限公司，专注一站式AI健康管理五年多。现面向全国招募社区健康服务中心项目合伙人。如果你对健康管理感兴趣，想低门槛撬动高价值、高利润项目，友福就是你的最佳选择。友福三大核心优势，帮合伙人轻松开拓市场。',auto_run:true,bgm_enabled:false,bgm_source:'manual',bgm_volume:0.25,bgm_ducking:true,bgm_fade_in:1.5,bgm_fade_out:2}} },
  computed: {
  ...
  methods: {
    ...
    async chooseRandomProjectBgm(){try{this.current=await request(`/api/projects/${this.current.id}/bgm/random`,{method:'POST'});this.resetVideo();this.toast(`已随机选择：${this.current.bgm_name||'背景音乐'}，请生成配乐版`,'success')}catch(error){this.toast(error.message)}},
