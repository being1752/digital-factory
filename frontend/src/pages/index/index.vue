    ...
    <view class="shell">
      ...
      <view class="workspace">
        ...
        <template v-else>
          ...
          <view v-if="current.has_video" class="panel content-card"><view class="section-head"><text>成品视频</text><button class="ghost small" @click="downloadVideo">下载视频</button></view><view v-if="!videoSource" class="video-lazy"><text class="video-lazy-title">视频尚未加载</text><text class="hint">切换项目不会下载视频，点击播放后才开始加载。</text><button class="primary small" @click="loadVideo">播放视频</button></view><video v-else class="video" :src="videoSource" controls autoplay @error="videoLoadError"></video></view>
...
const BUSY = new Set(['QUEUE_WAITING','UPLOADING_ASSETS','ANALYZE_QUEUED','ANALYZING_IMAGE','ANALYSIS_RETRYING','AUDIO_QUEUED','UPLOADING_REFERENCE_AUDIO','GENERATING_AUDIO','ALIGN_QUEUED','ALIGNING_SPEECH','PLANNING_ACTIONS','VIDEO_QUEUED','UPLOADING_VIDEO_ASSETS','GENERATING_VIDEO'])
...
export default {
  data() { return {health:{},appSettings:{comfy_url:''},settingsSaving:false,projects:[],tasks:[],current:null,poll:null,queuePoll:null,submitting:false,comfyChecking:false,comfyCheckResult:'',comfyCheckOk:false,imagePath:'',imageFileName:'',voicePath:'',voiceFileName:'',emotionVoicePath:'',emotionVoiceFileName:'',audioPlaying:false,audioCurrent:0,audioTotal:0,audioSource:'',videoSource:'',emotionNames:['Happy','Angry','Sad','Fear','Hate','Low','Surprise','Neutral'],form:{title:'健康管理口播',tts_engine:'indextts2_legacy',original_script:'百万亿健康管理蓝海市场，机遇就在眼前。友福同享智能科技有限公司，专注一站式AI健康管理五年多。现面向全国招募社区健康服务中心项目合伙人。如果你对健康管理感兴趣，想低门槛撬动高价值、高利润项目，友福就是你的最佳选择。友福三大核心优势，帮合伙人轻松开拓市场。',auto_run:true}} },
  computed: {
  ...
  methods: {
    ...
    async connectBackend(){try{this.health=await request('/api/health');await Promise.all([this.loadAppSettings(),this.loadProjects(),this.loadTasks()]);this.startQueuePolling()}catch(e){this.toast(e.message)}},
    ...
    loadVideo(){this.videoSource=this.assetUrl('video')},
