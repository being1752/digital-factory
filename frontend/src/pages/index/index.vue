    ...
    <view class="shell">
      <view class="sidebar panel">
        ...
        <view v-for="task in tasks" :key="task.id" class="queue-item" :class="`queue-${taskDisplayClass(task)}`" @click="openProject(task.project_id)">
          <view class="queue-main"><text class="project-name">{{ task.project_title }}</text><text class="project-meta">{{ statusName(task.display_status) }} · {{ taskStatusContext(task) }} · {{ Number(task.display_progress || 0) }}%</text><text class="project-meta">{{ ttsEngineName(task.snapshot?.tts_engine) }}</text><text v-if="task.stage==='ANALYSIS_RETRYING'&&task.error" class="queue-retry-error">第 {{ task.error.attempt }} 次失败，3 秒后重试：{{ task.error.message }}</text><text v-if="task.queue_position" class="queue-position">等待第 {{ task.queue_position }} 位</text></view>
          <button v-if="task.status==='QUEUED'||task.status==='RUNNING'" class="queue-action" @click.stop="cancelTask(task)">取消</button>
          <button v-if="canRetryTask(task)" class="queue-action retry" @click.stop="retryTask(task)">重试</button>
          <button class="queue-action delete" :disabled="task.deleting" @click.stop="deleteTask(task)">{{ task.deleting ? '删除中' : '删除' }}</button>
        </view>
      ...
      <view class="workspace">
        <view v-if="!current" class="panel create-card">
          ...
          <view v-if="form.subtitle_enabled||form.video_title_enabled" class="subtitle-settings">
            <view v-if="form.video_title_enabled" class="video-title-editor">
              <view class="subtitle-preset-bar"><view><text class="auto-run-title">视频号顶部标题</text><text class="hint">白色主标题＋黄色第二行 · 88号粗体 · 黑色描边 · 距顶部10%</text></view><button class="ghost small" @click="applyChannelsTitlePreset(form,form.original_script)">应用预设</button></view>
              <view><text class="label">标题文案（最多两行，每行12字）</text><textarea v-model="form.video_title" class="textarea video-title-input" :maxlength="100" placeholder="第一行主标题&#10;第二行亮点"></textarea><button class="ghost small title-regenerate" @click="form.video_title=suggestVideoTitleText(form.original_script)">从口播稿重新提取</button></view>
              <view class="subtitle-grid"><view><text class="label">字体</text><picker :range="fontOptions" @change="form.video_title_font_name=fontOptions[$event.detail.value]"><view class="field picker-field">{{ form.video_title_font_name }}</view></picker></view><view><text class="label">字号 {{ Number(form.video_title_font_size||88) }}</text><slider :value="Number(form.video_title_font_size||88)" min="48" max="140" step="1" activeColor="#d7ff68" block-size="14" @changing="form.video_title_font_size=$event.detail.value"/></view><view><text class="label">距顶部 {{ Number(form.video_title_position||10) }}%</text><slider :value="Number(form.video_title_position||10)" min="3" max="35" step="1" activeColor="#d7ff68" block-size="14" @changing="form.video_title_position=$event.detail.value"/></view><view><text class="label">黑色描边 {{ Number(form.video_title_stroke_width||4) }}</text><slider :value="Number(form.video_title_stroke_width||4)" min="0" max="8" step="0.5" activeColor="#d7ff68" block-size="14" @changing="form.video_title_stroke_width=$event.detail.value"/></view><view><text class="label">主标题颜色</text><input v-model="form.video_title_primary_color" class="field color-field" maxlength="7" /></view><view><text class="label">第二行颜色</text><input v-model="form.video_title_secondary_color" class="field color-field" maxlength="7" /></view></view>
            </view>
            ...
            <view class="subtitle-preview-frame"><image v-if="imagePath" class="subtitle-preview-image" :src="imagePath" mode="aspectFill"/><view class="subtitle-safe-zone">视频号界面安全区</view><view v-if="form.video_title_enabled" class="video-title-preview" :style="videoTitlePositionStyle(form)"><text v-for="(line,index) in videoTitleLines(form.video_title)" :key="`form-title-${index}`" :style="videoTitleLineStyle(form,index)">{{ line }}</text></view><view v-if="form.subtitle_enabled" class="subtitle-preview-text" :style="[subtitlePreviewStyle(form),subtitlePreviewPositionStyle(form)]"><text>{{ form.original_script.slice(0, Number(form.subtitle_max_chars||14)) || '字幕效果预览' }}</text></view></view>
          </view>
        ...
        <template v-else>
          ...
          <view class="panel content-card source-material-card">
          ...
          <view v-if="current.has_video" class="panel content-card">
            <view class="section-head"><text>成品视频</text><view class="actions"><button v-if="current.has_subtitle" class="ghost small" @click="downloadVideo('subtitle')">下载SRT</button><button v-if="current.has_raw_video" class="ghost small" @click="downloadVideo('video_raw')">下载无字幕原片</button><button v-if="current.has_subtitle_video" class="ghost small" @click="downloadVideo('video_subtitled')">下载字幕版</button><button v-if="current.has_bgm_video" class="ghost small" @click="downloadVideo('video_bgm')">下载配乐成片</button></view></view>
            ...
            <view v-if="current.has_raw_video" class="subtitle-settings subtitle-editor">
              <view class="subtitle-preset-bar"><view><text class="auto-run-title">视频号顶部标题</text><text class="hint">常驻两行标题与逐句字幕一次烧录，不重复压制</text></view><button class="ghost small" @click="applyChannelsTitlePreset(current,current.script||current.original_script)">应用预设</button></view>
              ...
              <view class="section-head"><view><text class="subheading quality-title">字幕后期</text><text class="hint inline-hint">修改样式不会重新生成数字人视频</text></view><button v-if="current.has_subtitle" class="ghost small" @click="downloadVideo('subtitle')">下载SRT</button></view>
...
export default {
  data() { return {health:{},appSettings:{comfy_url:''},settingsSaving:false,availableFonts:[],projects:[],tasks:[],current:null,poll:null,queuePoll:null,submitting:false,comfyChecking:false,comfyCheckResult:'',comfyCheckOk:false,imagePath:'',imageFileName:'',voicePath:'',voiceFileName:'',emotionVoicePath:'',emotionVoiceFileName:'',bgmPath:'',bgmFileName:'',audioPlaying:false,audioCurrent:0,audioTotal:0,audioSource:'',videoSource:'',emotionNames:['Happy','Angry','Sad','Fear','Hate','Low','Surprise','Neutral'],form:{title:'健康管理口播',tts_engine:'indextts2_legacy',original_script:'百万亿健康管理蓝海市场，机遇就在眼前。友福同享智能科技有限公司，专注一站式AI健康管理五年多。现面向全国招募社区健康服务中心项目合伙人。如果你对健康管理感兴趣，想低门槛撬动高价值、高利润项目，友福就是你的最佳选择。友福三大核心优势，帮合伙人轻松开拓市场。',auto_run:true,bgm_enabled:false,bgm_volume:0.25,bgm_ducking:true,bgm_fade_in:1.5,bgm_fade_out:2}} },
  computed: {
  ...
  methods: {
    ...
    async openProject(id){clearInterval(this.poll);this.resetAudio();this.resetVideo();this.current=await request(`/api/projects/${id}`);this.startPolling()},
    ...
    newProject(){clearInterval(this.poll);this.resetAudio();this.resetVideo();this.current=null},
    ...
    suggestVideoTitleText(script){const text=String(script||'').replace(/\s+/g,'').trim();if(!text)return '';const sentences=text.split(/[。！？!?]+/).filter(Boolean),first=sentences[0]||text,clauses=first.split(/[，,；;：:]/).map(item=>item.trim()).filter(Boolean);let lines=clauses.slice(0,2);if(lines.length===1&&lines[0].length>12){const value=lines[0],point=Math.min(12,Math.max(6,Math.ceil(value.length/2)));lines=[value.slice(0,point),value.slice(point)]}if(lines.length===1&&sentences.length>1)lines.push(sentences[1]);return lines.slice(0,2).map(line=>line.slice(0,12)).join('\n')},
    videoTitleLines(value){const lines=[];for(const raw of String(value||'').replace(/\r/g,'').split('\n')){let line=raw.replace(/\s+/g,'');while(line&&lines.length<2){lines.push(line.slice(0,12));line=line.slice(12)}if(lines.length>=2)break}return lines.length?lines:['视频标题']},
    applyChannelsTitlePreset(target,script){Object.assign(target,{video_title_enabled:true,video_title_font_name:'Microsoft YaHei',video_title_font_size:88,video_title_primary_color:'#FFFFFF',video_title_secondary_color:'#FFD84D',video_title_position:10,video_title_stroke_color:'#000000',video_title_stroke_width:4});if(!String(target.video_title||'').trim())target.video_title=this.suggestVideoTitleText(script);this.toast('已应用视频号顶部标题预设','success')},
    videoTitlePositionStyle(value){return {top:`${Math.max(3,Math.min(35,Number(value?.video_title_position??10)))}%`}},
    videoTitleLineStyle(value,index){const width=Math.max(0,Number(value?.video_title_stroke_width??4)/4.5),stroke=String(value?.video_title_stroke_color||'#000000');return {display:'block',fontFamily:String(value?.video_title_font_name||'sans-serif'),fontSize:`${Math.max(14,Number(value?.video_title_font_size||88)/4.5)}px`,fontWeight:'800',lineHeight:'1.2',color:index===0?String(value?.video_title_primary_color||'#FFFFFF'):String(value?.video_title_secondary_color||'#FFD84D'),WebkitTextStroke:width?`${width}px ${stroke}`:'0',paintOrder:'stroke fill'}},
